...
function isAlgorithmAssignmentMethod(assignmentMethod?: string | null) {
  return (assignmentMethod ?? "Manual").toLowerCase() !== "manual"
}

function getManualPrefilledSlots(rosterRows: RosterRow[]) {
  return rosterRows.flatMap((row) =>
    Object.values(row.shifts)
      .filter((shift): shift is ShiftAssignment => {
        if (!shift) return false
        return !isAlgorithmAssignmentMethod(shift.assignmentMethod)
      })
      .map((shift) => ({
        nurseId: row.nurseId,
        shiftDate: moment(shift.shiftDate).format("YYYY-MM-DD"),
...
function RosterPlanningPage() {
  ...
  const hasAutoGeneratedRoster = useMemo(() => {
    return rosterEntries.some((entry) =>
      isAlgorithmAssignmentMethod(entry.assignment_method),
    )
  }, [rosterEntries])
  ...
  const hasAutoGeneratedRosterData = useMemo(
    () =>
      rosterData.some((row) =>
        Object.values(row.shifts ?? {}).some(
          (shift) =>
            shift != null &&
            isAlgorithmAssignmentMethod(shift.assignmentMethod),
        ),
      ),
    [rosterData],
  )
  const showAlgorithmGeneratedState =
    isAlgorithmGenerated && hasAutoGeneratedRosterData
  ...
  const handleGenerateAlgorithm = useCallback(async () => {
    ...
    try {
      ...
      const result = await generateAlgorithmRoster.mutateAsync({
        ...
        prefilledSlots: getManualPrefilledSlots(rosterData),
        onProgress: (percent) => {
      ...
      const rows: RosterRow[] = mock.roster.nurses.map((nurse) => {
        ...
        nurse.schedule.forEach((shiftCode, index) => {
          ...
          shiftsObject[dateKey] = {
            ...
            status: "Pending",
            assignmentMethod: mock.method ?? "Algorithm",
          }
        })
