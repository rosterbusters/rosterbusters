...
function getGroupedShiftCount(
  ...
  return data.reduce((sum, row) => {
    ...
    return mapShiftCodeToSummaryType(shift.shiftCode) === shiftType
      ? sum + 1
      : sum
  }, 0)
}
    ...
    <>
              ...
              <Table.Row key={group.key}>
                ...
                {dayColumns.map((col) => {
                  ...
                  const effectiveGuidelines =
                    dateOverrides[dateKey] ?? guidelines
                      ...
                      <Flex>
                        {visibleShiftTypes.map((shiftType) => {
                          ...
                          const isModified =
                            editableRole !== null &&
                            (modifiedCells.has(
                              `${editableRole}-${shiftType}`,
                            ) ||
                              modifiedCells.has(
                                `${dateKey}-${editableRole}-${shiftType}`,
                              ))
                          ...
                          return (
                            ...
                            >
                              <Flex
                                ...
                                outline={
                                  isModified ? "2px solid #4CAF50" : undefined
                                }
                                outlineOffset="-2px"
                                _hover={
                                  isEditable
                                  ...
                                  isEditable
                                    ? () =>
                                        setSelectedCell({
                                          ...
                                          shiftType,
                                          dateKey,
                                          dateLabel: moment(col.date).format(
                                            "ddd, MMM D",
                                          ),
                                        })
                                    : undefined
      ...
      {selectedCell && (
        <ManpowerEditDialog
          ...
          currentMin={
            (dateOverrides[selectedCell.dateKey] ?? guidelines)[
              selectedCell.role
            ][selectedCell.shiftType].minimum
          }
          currentMax={
            (dateOverrides[selectedCell.dateKey] ?? guidelines)[
              selectedCell.role
            ][selectedCell.shiftType].maximum
          }
          originalMin={
            (originalGuidelines ?? guidelines)[selectedCell.role][
